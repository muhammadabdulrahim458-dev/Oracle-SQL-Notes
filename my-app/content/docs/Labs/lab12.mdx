...
| :---       | :---         | :---     | :---       |
| 1          | John Doe     | New York | 2024-01-15 |
| 2          | Alice Smith  | London   | 2024-03-20 |
| 3          | Bob Johnson  | Paris    | 2024-05-10 |
| 4          | Clara Lee    | Tokyo    | 2024-07-01 |
| 5          | David Brown  | Berlin   | 2024-08-15 |
| 6          | Emma Wilson  | Rome     | 2025-01-01 |
...
| :---      | :---        | :---         | :---   | :---  | :---       |
| 1         | Laptop      | Electronics  | 1200.00| 10    | 1          |
| 2         | Smartphone  | Electronics  | 800.00 | 15    | 1          |
| 3         | Monitor     | Electronics  | 300.00 | 20    | 1          |
| 4         | Tablet      | Electronics  | 500.00 | 10    | 1          |
| 5         | Desk        | Furniture    | 450.00 | 5     | 2          |
| 6         | Chair       | Furniture    | 250.00 | 10    | 2          |
| 7         | Bookshelf   | Furniture    | 180.00 | 8     | 3          |
...
| :---    | :---       | :---       | :---        | :---      |
| 1       | 1          | 2024-02-10 | 1950.00     | Shipped   |
| 2       | 2          | 2024-04-15 | 1300.00     | Delivered |
| 3       | 3          | 2024-06-20 | 950.00      | Shipped   |
| 4       | 1          | 2024-07-25 | 800.00      | Delivered |
| 5       | 4          | 2024-09-01 | 1320.00     | Pending   |
| 6       | 5          | 2024-10-10 | 265.00      | Shipped   |
| 7       | 2          | 2024-11-05 | 300.00      | Delivered |
| 8       | 3          | 2024-12-15 | 700.00      | Shipped   |
| 9       | 4          | 2025-01-20 | 950.00      | Delivered |
| 10      | 5          | 2025-02-25 | 2000.00     | Shipped   |
...
### 3. SQL Query Questions (Mixed Difficulty)

1. Write a query to retrieve the `OrderID`, `OrderDate`, and the corresponding `CustomerName` for every order in the system.

2. Write a query to find the `ProductName` and `Price` of the single most expensive product in the inventory.

3. Write a query to display the `ProductName` and the total quantity sold (sum of `Quantity` from `OrderItems`) for each product, sorted from most sold to least sold.

4. Write a query to show the `OrderID`, `CustomerName`, `ProductName`, and `Quantity` for all orders placed after `'2025-01-01'`.

5. Find the names of all customers who have never placed an order.

6. Write a query to find all products whose `Price` is strictly greater than the average price of products within their own `Category`.

7. Find customers who have placed more than 5 orders. Display their `CustomerName` and the total number of orders they have placed.

8. Identify the suppliers who supply every single product that belongs to the `'Electronics'` category.

9. Write a query to display each `OrderID`, its `TotalAmount`, and the historical average `TotalAmount` of all orders placed by that specific customer.

10. List the names of customers who have spent a total (`SUM(TotalAmount)` across all their orders) that is greater than the overall average total spending of all customers in the system.

11. Write a query to find the product with the second-highest price within each product category. Display `Category`, `ProductName`, and `Price`.

12. Find all customers who have placed an order containing the product that has the maximum price among all products in the entire catalog.
...
### Solution 9
*Using a scalar subquery in the SELECT clause (traditional exam style):*
```sql
SELECT o.OrderID,
       o.TotalAmount,
       (SELECT AVG(o2.TotalAmount)
        FROM Orders o2
        WHERE o2.CustomerID = o.CustomerID) AS CustomerAvgOrderValue
FROM Orders o;
```

*For completeness – Oracle analytic equivalent (even cleaner):*
```sql
SELECT OrderID,
       TotalAmount,
       AVG(TotalAmount) OVER (PARTITION BY CustomerID) AS CustomerAvgOrderValue
FROM Orders;
```
